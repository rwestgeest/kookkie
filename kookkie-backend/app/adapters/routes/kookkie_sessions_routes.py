...
from app.domain import ID, KookkieSession, JoinInfo, KookkieSessionListItem
...
class KookkieSessionRoutes(object):
    ...
    def register(self, application):
        ...
        def all_kookkie_sessions():
            return build_response(from_result_of(Success(kookkies=self.kookkieSessionsRepository.all(kook=self.current_user_repository.current_user())))
                    .on_success(lambda result: (as_kookkie_list(result.kookkies), 200))
                    .with_csrf())
...
def as_kookkie_list(kookies: List[KookkieSessionListItem]):
    return dict(kookkies=list(map(as_kookkie_session, kookies)))
